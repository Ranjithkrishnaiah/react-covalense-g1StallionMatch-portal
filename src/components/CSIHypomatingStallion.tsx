...
type PedigreeProps = {
 ...
 function CSIHypoMatingStallion(props: any) { 
   ...
   const [hId, setHId] = React.useState<any>({
    ...
    // Open the pedigree modal if user clicks on the horse name
            ...
            <Box sx={ { height: '100%' } } className='Pedigree'>
                  {genArr?.map((pedigree: PedigreeProps, subindex: number) => (
                     <Box style={ { width: '100%' } } key={subindex} className='treeCell'>
                        ...
                        onClick={() => {(index > 0) ? void(0) : handlePedigree(stallionId, 'stallion', props?.stallionFarmCount, pedigree?.isRaceHorse, pedigree?.raceHorseUrl)}}
                        >                              
 ...
 export default CSIHypoMatingStallion;