...
export default function MultipleSelectChip(props: any) {
  ...
  // set name base on placeholder
  React.useEffect(()=>{
    if(props.searchColour?.length > 0 && props?.placeholder === 'Colour') {
      let newChipValueArray = null;
          newChipValueArray = colours?.filter((colour: any) => props.searchColour?.includes(colour?.id));
          let labelValues = newChipValueArray?.map((colourObject: any) => colourObject.label);
          setName(labelValues);
    }  
  ...
  React.useEffect(()=>{
    if(props.searchYearOfStudKey?.length > 0 && props?.placeholder === 'Year of Stud' ){
      setName(props?.searchYearOfStudKey);
    }
  },[props.searchYearOfStudKey?.length])

 // set name base on placeholder
  React.useEffect(()=>{
    if(props.searchFarmKey?.length > 0 && props?.placeholder === 'Farm'){
      let newChipValueArray = null;
      newChipValueArray = farms?.filter((farm: any) => props.searchFarmKey?.includes(farm?.id));
      let labelValues : any = newChipValueArray?.map((farmObject: any) => farmObject.label);
      setName(labelValues);
    }  
  },[props.searchFarmKey?.length])
              ...
              >
                {selected.map((value: React.ReactNode, index: number) => (
                  <Chip
                    ...
                    label={value}
                    clickable
                    deleteIcon={<i className='icon-Cross' onMouseDown={(e) => handleDelete(e, value)} style={ {color:'#161716'} } />}
          ...
          >
            {list[props?.placeholder] &&
              list[props?.placeholder].map((res: any) => (
                ...
                <Checkbox checkedIcon={<img src={Images.checked} alt="checkbox"/>} icon={<img src={Images.unchecked} alt="checkbox"/>} checked={name.indexOf(res.label) > -1} />
                </MenuItem>
