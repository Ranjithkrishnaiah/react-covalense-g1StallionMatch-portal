...
import { useAutosearchStallionNamesQuery, useStallionNamesListForFarmsQuery } from 'src/redux/splitEndpoints/searchStallionNamesSplit';
...
function StallionListAndMareSearch(props: any) {
    ...
    // Stallion search API call
    const {
        data: stallionNamesListForFarms,
    } = useStallionNamesListForFarmsQuery(props.farmId);
                    ...
                    <Stack className="SMareWrapper">
                        ...
                        >
                            ...
                            <Autocomplete
                                ...
                                options={stallionNamesListForFarms || []}
                                noOptionsText={
                                    ...
                                    <Box className="search-couldnot d-flex align-items-center justify-content-between mt-2 scroll-submit-stallion-wrp">
                                        <span className="fw-bold sorry-message">
                                            {stallionNamesListForFarms?.length === 0 && `Sorry, we couldn't find any stallion`}
                                        </span>
                                ...
                                filterOptions={(option: any, state: any) => {
                                    ...
                                    optionList = option?.filter((v: any) => {
                                        let stallionFullname = v?.stallionName?.toLowerCase();
                                        let searchStallionName = state?.inputValue?.toLowerCase();
                                        if (stallionFullname?.startsWith(searchStallionName)) {
                                            return true;
                                        }
                                        return false;
                                    })
...
export default StallionListAndMareSearch;