import { FormControlLabel, MenuItem, SelectChangeEvent, Box, TextField, Typography } from '@mui/material';
...
} from 'src/redux/splitEndpoints/getCurrenciesSplit';
import {
  Checkbox
} from '@mui/material';
...
export default function FeeRange(props: any) {
  ...
  useEffect(() => {
    ...
    let comeFromDirectory: any = window.localStorage.getItem('comeFromFarmDirectory');
    if (value) {
  ...
  let onMouseUpInputPriceChange = (e: any) => {
    if (!selectedPriceRange[1] && !(selectedPriceRange[0] > selectedPriceRange[1])) {
    }
  }
